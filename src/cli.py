...
def retrieve(question, k=None, cfg=None, rerank=False, _cache={}):
    ...
    rerank=True applies the MaxSim late-interaction mitigation: pull a wider top-50 by pooled
    cosine, then re-order those by token-level max-similarity (see src/rerank.py). the score
    shown then is the MaxSim score, not the pooled cosine.
    """
    ...
    if rerank:
        from src.rerank import rerank_query
        cand, _ = top_k(scores, 50)
        ranked, ms_scores = rerank_query(embedder, question, cand, chunks)
        idx = ranked[:k]
        sc = ms_scores[:k]
    else:
...
def print_hits(question, hits, answer, score_label="cosine"):
    print(f'\nquery: "{question}"')
    # spelled out as extractive on purpose: this is the best-matching sentence lifted verbatim
    # from chunk #1 below, not text generated by a model. nothing here can hallucinate.
    print(f"\nanswer (extractive, from chunk #1): {answer}\n")
    ...
    for h in hits:
        ...
        print(f"  #{h['rank']}  {score_label} {h['score']:.4f}   {h['title']}")
...
def main():
    ...
    ap.add_argument("--rerank", action="store_true",
                    help="apply the MaxSim late-interaction mitigation (top-50 -> re-rank)")
    args = ap.parse_args()

    cfg = load_config()
    hits, embedder = retrieve(args.question, k=args.k, cfg=cfg, rerank=args.rerank)
    answer = best_sentence(args.question, hits[0]["text"], embedder)

    print_hits(args.question, hits, answer, score_label="maxsim" if args.rerank else "cosine")
    if args.debug:
